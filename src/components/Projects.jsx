...
const ProjectItem = ({ project, index, isVisible }) => {
  ...
  useEffect(() => {
    if (isVisible) {
      // Removed horizontal offset animation
      setTimeout(() => {
        itemControls.start({
          opacity: 1,
          y: 0, // Now using vertical fade-in only
          transition: { 
            duration: 0.6, 
      ...
      }, index * 120); // Fast staggering for fluid appearance
    } else {
      setTimeout(() => {
        itemControls.start({
          opacity: 0,
          y: 15, // Small vertical offset for exit
          transition: { 
            duration: 0.4,
            ease: "easeInOut"
          }
  ...
  return (
    <motion.div
      initial={{ opacity: 0, y: 15 }} // Changed from x to y offset
      animate={itemControls}
      className="p-6 rounded-lg shadow-lg backdrop-blur-lg bg-gray-900 bg-opacity-20 hover:bg-opacity-30 transition-all duration-500"
      whileHover={{ 
        scale: 1.02,
    ...
    >
      ...
      <motion.h3 
        ...
        whileHover={{ color: "#38b2ac", scale: 1.01, transition: { duration: 0.3 } }} // Removed x movement
      >
        {project.title}
      </motion.h3>
      <motion.p 
...
const Projects = () => {
  ...
  // Modified animation variants (removed horizontal movement)
  ...
  // Removing horizontal offset animation from title
  const titleVariants = {
    hidden: { opacity: 0, y: 15 }, // Changed from x to y offset
    visible: { 
      opacity: 1, 
      y: 0, 
      transition: { 
